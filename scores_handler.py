    ...
    def main_scores(id_user, *args):
        ...
        for string_split in split_maket:
            ...
            if key_id == 'Переклеїв' and sum_glass_count != 0:
                ...
                string_split += ' {} ({:.3})'.format(count_instyle, float(sum_instyle_job))
            elif 'Видано клієнтських' in string_split:
                # string_split += f' {count_client} ({round(sum_client_job, 2)})'
                string_split += ' {} ({:.3})'.format(count_client, float(sum_client_job))
        ...
        result_maket.append('Загальний результат - {:.4}'.format(float(sum_user_job)))
        final_maket = '\n'.join(result_maket)
